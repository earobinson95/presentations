...
```{r linear-regression, echo = F, fig.width = 6, fig.height = 9/2, out.width = "75%", fig.align='center'}
dat <- mtcars
...
```{r linear-regression2, echo=F, fig.align='center', fig.height=9/2, fig.width=6, message=FALSE, warning=FALSE, out.width="75%"}
dat <- mtcars
...
ggplot(dat, aes(x = wt, y = mpg)) +
  ...
  geom_line(aes(x = wt, y = 35-6.5*(wt-1)), color = "steelblue") +
  geom_line(aes(x = wt, y = 30-5*(wt-1)), color = "steelblue") +
  geom_line(aes(x = wt, y = 27.5-3.5*(wt-1)), color = "steelblue") +
  theme_minimal() +
  scale_y_continuous(limits = c(5,40), breaks = seq(5,40,5))
```
