...
# logoleft_name: '![](images\manuscript-qr-codes.png){.main-img-left}'
# logoright_name: '![](images\youdrawit-mobile-qr-code-text.png){.main-img-right}'
# POSTER BODY OPTIONS
...
eyefitting.gamm.plot <- eyefitting.grid.gamm %>%
  ...
  scale_y_continuous("Residual") +
  scale_color_manual("Individual participant \nresiduals", values = c("steelblue", "orange"), labels = c("OLS", "PCA"), guide = guide_legend(override.aes = list(alpha = 1, size = 1.5))) +
  scale_fill_manual("GAMM fitted trend", values = c("steelblue", "orange"), labels = c("OLS", "PCA"), guide = guide_legend(override.aes = list(alpha = 1))) 
```
...
A total of 35 individuals completed 119 unique 'You Draw It' task plots. For each participant and parameter choice, the feedback data set contained $x, y_{drawn}, \hat y_{OLS}$, and $\hat y_{PCA}$.
